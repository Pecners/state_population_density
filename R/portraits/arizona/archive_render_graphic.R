...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("arizona", "_", " ")))
...
st_dd <- st_read("data/kontur_population_US_20231101.gpkg",
                wkt_filter = wkt_st)
...
fc <- lighten(colors[10], .75)
colors_f <- c(colors, fc)
swatchplot(colors_f)

texture <- grDevices::colorRampPalette(colors_f |> rev(), bias = .75)(256)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(230, 230, 240, 240)),
    lightcolor = c(colors[2], "white", colors[10], "white"),
